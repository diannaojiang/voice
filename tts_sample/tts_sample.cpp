...
{
	int         ret = MSP_SUCCESS;
	const char* login_params = "appid = 57d9615b, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
																/*
																* rdn:           合成音频数字发音方式
																* volume:        合成音频的音量
																* pitch:         合成音频的音调
...
#define MAX_LENGTH 256
	string file;
	cout << endl << "请在当前目录下的text.txt文件中放入utf-8格式的文字\n";
	getchar();
	ifstream in("text.txt", ios::in);
	istreambuf_iterator<char> beg(in), end;
	string s(beg, end);
	in.close();
	cout << endl << "请输入要生成的文件名称：";
	cin >> file;
	file = file + ".wav";
	const char* filename = file.c_str(); //合成的语音文件名称
	const char *text = s.c_str(); //合成文本
								  /* 用户登录 */
